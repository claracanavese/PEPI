...
input <- lapply(t, function(time) {
  dplyr::tibble(
    zmin = zmin_ode(time, z0 = z0, lambda_minus = 1.2, lambda_plus = 1.2, omega_minus = .01, omega_plus = .001),
    zplus = zplus_ode(time, z0, lambda_minus = 1.2, lambda_plus = 1.2, omega_minus = .01, omega_plus = .001)
    )
}) %>% do.call('bind_rows', .)
output <- lapply(t, function(time) {
  dplyr::tibble(
    zmin = zmin_ode(time, z0 = z0, lambda_minus = 1.2, lambda_plus = 1.19, omega_minus = 0.0071, omega_plus = 0.001),
    zplus = zplus_ode(time, z0, lambda_minus = 1.2, lambda_plus = 1.19, omega_minus = 0.0071, omega_plus = 0.001)
  )
}) %>% do.call('bind_rows', .)
...
compmin = ggplot() +
  ...
  geom_point(data = simulation2, aes(x = time, y = z_minus, color = "sim2"), size = 1)
...
compplus = ggplot() +
  ...
  geom_point(data = simulation2, aes(x = time, y = z_plus, color = "sim2"), size = 1) 
