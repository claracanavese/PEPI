...
out1a <- ode(y = state2, times = seq(0, 40, by = 0.01), func = covariances2, parms = parameters_cov1a)
out1b <- ode(y = state2, times = seq(0, 40, by = 0.01), func = covariances2, parms = parameters_cov1b)
out1c <- ode(y = state2, times = seq(0, 40, by = 0.01), func = covariances2, parms = parameters_cov1c)
out1a_df <- data.frame(t = out1a[,1], M1 = out1a[,2], M2 = out1a[,3], V1 = out1a[,4], V2 = out1a[,5], C = out1a[,6])
out1b_df <- data.frame(t = out1b[,1], M1 = out1b[,2], M2 = out1b[,3], V1 = out1b[,4], V2 = out1b[,5], C = out1b[,6])
out1c_df <- data.frame(t = out1c[,1], M1 = out1c[,2], M2 = out1c[,3], V1 = out1c[,4], V2 = out1c[,5], C = out1c[,6])
...
out2a <- ode(y = state2, times = seq(0, 40, by = 0.01), func = covariances2, parms = parameters_cov2a)
out2b <- ode(y = state2, times = seq(0, 40, by = 0.01), func = covariances2, parms = parameters_cov2b)
out2c <- ode(y = state2, times = seq(0, 40, by = 0.01), func = covariances2, parms = parameters_cov2c)
out2a_df <- data.frame(t = out2a[,1], M1 = out2a[,2], M2 = out2a[,3], V1 = out2a[,4], V2 = out2a[,5], C = out2a[,6])
out2b_df <- data.frame(t = out2b[,1], M1 = out2b[,2], M2 = out2b[,3], V1 = out2b[,4], V2 = out2b[,5], C = out2b[,6])
out2c_df <- data.frame(t = out2c[,1], M1 = out2c[,2], M2 = out2c[,3], V1 = out2c[,4], V2 = out2c[,5], C = out2c[,6])
...
out3a <- ode(y = state2, times = seq(0, 40, by = 0.01), func = covariances2, parms = parameters_cov3a)
out3b <- ode(y = state2, times = seq(0, 40, by = 0.01), func = covariances2, parms = parameters_cov3b)
out3c <- ode(y = state2, times = seq(0, 40, by = 0.01), func = covariances2, parms = parameters_cov3c)
out3a_df <- data.frame(t = out3a[,1], M1 = out3a[,2], M2 = out3a[,3], V1 = out3a[,4], V2 = out3a[,5], C = out3a[,6])
out3b_df <- data.frame(t = out3b[,1], M1 = out3b[,2], M2 = out3b[,3], V1 = out3b[,4], V2 = out3b[,5], C = out3b[,6])
out3c_df <- data.frame(t = out3c[,1], M1 = out3c[,2], M2 = out3c[,3], V1 = out3c[,4], V2 = out3c[,5], C = out3c[,6])
...
plotsm1b + plotsm2b + plotsm3b
ggsave("./variance_p1PRL.png",  width = 10, height = 3, dpi = 600)
...
input <- lapply(t, function(time) {
  dplyr::tibble(
    zmin = zmin_ode(time, z0 = z0, lambda_minus = 1, lambda_plus = 1.5, omega_minus = 0.01, omega_plus = 0.001),
    zplus = zplus_ode(time, z0, lambda_minus = 1, lambda_plus = 1.5, omega_minus = 0.01, omega_plus = 0.001)
    )
}) %>% do.call('bind_rows', .)
output <- lapply(t, function(time) {
  dplyr::tibble(
    zmin = zmin_ode(time, z0 = z0, lambda_minus = 1.01084, lambda_plus = 1.53423, omega_minus = 0.00793, omega_plus = 0.00125),
    zplus = zplus_ode(time, z0, lambda_minus = 1.01084, lambda_plus = 1.53423, omega_minus = 0.00793, omega_plus = 0.00125)
  )
}) %>% do.call('bind_rows', .)

sim <- read.csv("./PEPI/sim_1.2_0.001_0.01/simulation_3 copy.csv")
...
compmin = ggplot() +
  ...
  geom_line(data = output, aes(x = t, y = zmin, color = "inf_min"), size = 0.8) + xlim(6,10) +
  #geom_point(data = simulation1, aes(x = time, y = z_minus, color = "sim1"), size = 1) + xlim(6,10) +
  geom_point(data = simulation3, aes(x = time, y = z_minus, color = "sim3"), size = 1)
...
compplus = ggplot() +
  ...
  geom_line(data = output, aes(x = t, y = zplus, color = "inf_plus"), size = 0.8) + xlim(6,10) +
  #geom_point(data = simulation1, aes(x = time, y = z_plus, color = "sim1"), size = 1) + xlim(6,10) +
  geom_point(data = simulation3, aes(x = time, y = z_plus, color = "sim3"), size = 1) 
