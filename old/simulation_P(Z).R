...
final1 <- final1[,2:4]
final2 <- read.csv("./pz_sims/1.2_0.01_0.001_8t_81p.csv")
final2 <- final2[,2:4]
final3 <- read.csv("./pz_sims/1.0_1.5_0.01_0.001_8t_81p.csv")
final3 <- final3[,2:4]
...
rate2 <- Pz_exp(final2,1.2,1.2,0.01,0.001)
...
gamma <- function(t,lm,lp,om,op){
  gamma = cos(lm/lp*pi/2)*om/(lp-lm)*(op*pi/(lp*sin(pi*lm/lp)))^(lp/lm)*exp(t*lp+op*om/(lp-lm)*t)
  return(gamma)
}
delta = exp(8)

ggplot() +
  stat_function(fun = dstable, args = list(alpha = 1/1.5, beta = 1, gamma = gamma(8,1,1.5,0.001,0.01), delta = delta), linewidth=0.6) + 
  xlim(0,1e4) + ylim(0,0.0015)
...
powerlaw_coeff_plus <- function(alpha_min,alpha_plus,lambda_min,lambda_plus,omega_m,omega_p,time) {
  return(alpha_min/(lambda_plus*alpha_plus)*omega_p*pi/sin(pi*lambda_min/lambda_plus)*exp((lambda_min+omega_m*omega_p/(lambda_plus-lambda_min)*lambda_min/lambda_plus)*time)/gamma(1.0-lambda_min/lambda_plus))
}

powerlaw_coeff_minus <- function(alpha_min,alpha_plus,lambda_min,lambda_plus,omega_m,omega_p,time) {
  return((omega_m/(lambda_plus - lambda_min))^(lambda_min/lambda_plus)*powerlaw_coeff_plus(alpha_min,alpha_plus,lambda_min,lambda_plus,omega_m,omega_p,time))
}

powerlaw_coeff1 <- powerlaw_coeff_plus(alpha_min = 1.0,alpha_plus = 1.5,lambda_min = 1.0, lambda_plus = 1.5, omega_m = 0.01,omega_p = 0.001,time = 8)
powerlaw_coeff2 <- powerlaw_coeff_minus(alpha_min = 1.0,alpha_plus = 1.5,lambda_min = 1.0, lambda_plus = 1.5,omega_m = 0.01,omega_p = 0.001,time = 8)
...
plot1p <- final3 %>% 
  #filter(`Z+`<4e5) %>% 
  mutate(pw=powerlaw_coeff1*z_plus**(-(1+1.0/1.5))) %>%
  ggplot(aes(x=z_plus)) + 
...
plot1m <- final3 %>% 
  #filter(`Z+`<4e5) %>% 
  mutate(pw=powerlaw_coeff2*z_minus**(-(1+1.0/1.5))) %>%
  ggplot(aes(x=z_minus)) + 
...
# plot Z- VS Z+
final1a %>% 
  mutate(z = z_minus + z_plus, "Z- rate" = z_minus/z, "Z+ rate" = z_plus/z) %>% 
  ggplot(aes(x=z_minus,y=z_plus)) + 
  geom_point(color = 'cyan4') +
  geom_line(aes(x=z_minus, y=z_minus*omega_p/(alpha_min-alpha_plus)), color='black')
...
lambda_min=1.5; lambda_plus=1.0; omega_m=0.01; omega_p=0.001; alpha_min=1.5; alpha_plus=1; beta_min=0; beta_min=0
time = 8;
final1a <- read.csv("./pz_sims/1.5_1.0_0.01_0.001_8t_81p.csv")
final1a <- final1a[,2:4]
final1a_j = final1a %>% filter(z_plus < 2000 & z_plus > 1 & z_minus < 9e5)

ggplot() +
  geom_point(final1a_j,mapping = aes(x=z_minus, y=z_plus)) +
  stat_density_2d(joint_th, mapping = aes(x=x,y=y, fill = ..level..), geom = "polygon", alpha = 0.1, bins = 50) +
  scale_fill_continuous(type = "viridis") 
  #scale_fill_brewer()
  #theme_bw()

ggplot() +
  geom_point(final1a_j,mapping = aes(x=z_minus, y=z_plus), size=0.5) +
  stat_density_2d_filled(joint_th, mapping = aes(x=x,y=y), alpha = 0.4, bins = 7) +
  geom_density_2d(joint_th, mapping = aes(x=x,y=y),colour = "black", bins = 7) +
...
x <- rexp(1000, rate = exp(-(lambda_min + omega_m*omega_p/(lambda_min-lambda_plus))*time))
# x <- rpareto(1000,lambda_min/lambda_plus, scale = 10000)
y <- sapply(x, function(x) x*omega_p/(lambda_min - lambda_plus))

joint_th <- data.frame(x,y)

ggplot() +
  stat_density_2d(aes(x=rexp(1000,rate = (lambda_min-lambda_plus)/omega_p*exp(-(lambda_min+omega_m*omega_p/(lambda_min-lambda_plus))*8)),y=rexp(1000,rate = exp(-(lambda_min+omega_m*omega_p/(lambda_min-lambda_plus))*8)),fill= after_stat(level)))
